import { Component, OnInit, ElementRef, ViewChild, trigger, transition, style, animate, state } from '@angular/core';
...
@Component({
  selector: 'onboard-private_limited',
  templateUrl: './private_limited.component.html',
  styles: [`
  .not-active {
    pointer-events: none;
    cursor: default;
...
export class PrivateLimitedComponent implements OnInit {

  public businees_type: any[] = [];
  public businees_category: any[] = [];
  public address_proof_type: any[] = [];
  public gender = [];
  public companyListResult: any = [];
  public company_list: any[] = [];
  public mob_data: any = [];
  public address_proof: any = [];
  public state_list: any[] = [];
  public current_address_proofs_files_id: any = [];
  public operation_address_proofs: any = [];
  public operation_address_proofs_files_id: any = [];
  public director_details: any = [];
  //-----------------------------------------------------------//
  public onbord_flag; //1,2,3,4
  public onbord_form_flag; //1,2,3
  ...
  public users_id;
  //-----------------------------------------------------------//
  public businees: number = 4;
  public check_almost: number = 0;
  public pan_aadhar: number;
  //-----------------------------------------------------------//
  public webcam: any
  public error: any;
  public options: any;
  ...
  public pan_data: any;
  public aadhar_data: any;
  public aadhar_data_front: any;
  public aadhar_data_back: any;
  public businesspan_data: any;
  public registered_business_detail: any;
  public aadhaar_detail: any;
  public aadhar_e_data: any;
  public operation_address_data: any;
  public ucompanyName: any;
  //-----------------------------------------------------------//
  ...
  public showBusinessAddressForm: boolean = false;
  public showAadhaarUpdateDetailForm: boolean = false;
  public businessAddressProof: boolean = false;
  public stakeholder_address_textarea: boolean = true;
  public stakeHolderAadhaarAddressSuccess: boolean = false;
  public aadhaar_verified_status: boolean = true;
  ...
  public is_operation_addr_diff: boolean = false;
  ...
  //---------------------------------------------------------------------//
  public authorised_signatory_selected: string = "";
  public verification_popup_image: string = "";
  public pinkloader = "";
  public verification_popup_heading: string = "Verifying Details";
  public dateofincorporation_business_form: string = ''
  public selectedDirectorName: string;
  ...
  @ViewChild('closeAddSingatoryModel') closeAddSingatoryModel: ElementRef;
  @ViewChild('modalAccordion') modalAccordion: ElementRef;
  @ViewChild('verifydetails') verifydetails: ElementRef;
  @ViewChild('modalAccordionClose') modalAccordionClose: ElementRef;
  @ViewChild('verifydetailsClose') verifydetailsClose: ElementRef;
  //---------------------------------------------------------------------//
  ...
  business_detail = new FormGroup({
    ...
    registered_business_name: new FormControl('', [Validators.required]),
    date_of_incorporation: new FormControl(''),
    cin: new FormControl(''),
    brand_name: new FormControl('', [Validators.maxLength(100)]),
    business_pan: new FormControl('', [Validators.required, Validators.pattern('^[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}$')]),
  ...
  business_address = new FormGroup({
    ...
    operation_address: new FormGroup({
      // address_line: new FormControl('', [Validators.required]),
      // city: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z ]+')]),
      // state: new FormControl('', [Validators.required]),
      // pincode: new FormControl('', [Validators.required, Validators.pattern('^[0-9]{6}$')]),
    ...
    is_operational_address_different: new FormControl('')
  });


  add_signatory = new FormGroup({
    add_signatory_first_name: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z ]+$')]),
    add_signatory_middle_name: new FormControl('', [Validators.pattern('^[a-zA-Z ]+')]),
    add_signatory_last_name: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z ]+')]),
    add_signatory_gender: new FormControl('', [Validators.required]),
    add_signatory_dob: new FormControl('', [Validators.required]),
    add_signatory_pan_number: new FormControl('', [Validators.required, Validators.pattern('^[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}$')])
  });
  ...
  aadhaarUpdateDetail = new FormGroup({
    aadhaar_first_name: new FormControl(''),
    aadhaar_number: new FormControl(''),
    aadhaar_dob: new FormControl(''),
    aadhaar_gender: new FormControl('', [Validators.required]),
    aadhaar_address: new FormControl(''),
  });
  ...
  search(event) {
    if (localStorage.getItem('director_detail')) {
      localStorage.removeItem('director_detail');
    }
    this.companyDetail.status = false;
    this.loaderClass = 'addInputLoader';
    this._httpService.getCompanyList({ 'company_name': event.query })
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.data.length > 0) {
          this.companyListResult = result.data;
          //this.loaderClass = '';
        } else {
          this.companyListResult = [{ 'companyName': 'No data found' }];
          this.companyDetail.name = event.query;
          this.companyDetail.cin = '';
          this.dateofincorporation_business_form = this.companyDetail.doi = '';
  ...
  getCompanyDetail(event) {
    ...
    if (event.hasOwnProperty('companyID')) {
      this.companyDetail.status = true;
      this.loaderClassForRoc = 'addLoader';
      this._httpService.getCompanyDetail({ 'company_id': event.companyID })
        .subscribe((result) => {
          this.hideWarningClass = "clickHide";
          if (result.data) {
            this.companyDetail.name = result.data.companyName;
            this.companyDetail.doi = result.data.dateOfIncorporation;
            if (this.companyDetail.doi != '') this.dateofincorporation_business_form = this.companyDetail.doi;
            this.companyDetail.cin = result.data.cin;
            this.companyDetail.director_detail = result.data.directorDetails;
            localStorage.setItem('director_detail', JSON.stringify(result.data.directorDetails));
            let registerd_address = {
              "address_line": result.data.splitAddress.addressLine,
              "district": result.data.splitAddress.district[0],
              "city": result.data.splitAddress.city[0],
              "state": result.data.splitAddress.state[0][0],
              "pincode": result.data.splitAddress.pincode,
              "country": result.data.splitAddress.country[2]
            };
            this.companyDetail.registered_business_address = registerd_address;
            this.loaderClassForRoc = '';
          }
        },
        (err: any) => {
          if (err == "Error: 350") {
            ...
            this.loaderClassForRoc = '';
            this.companyDetail.status = false;
            this.companyDetail.cin = event.companyID;
            this.companyDetail.name = event.companyName;
            this.dateofincorporation_business_form = this.companyDetail.doi = '';
            this.companyDetail.registered_business_address = {};
            if (localStorage.getItem('director_detail')) {
              localStorage.removeItem('director_detail');
            }
          }
          if (err.status == 0) { this.hideWarningClass = ""; this.warning_message = "Please check your internet connection" }
          else if (err.status == 500) { console.log('oops something went wrong, please try again!!'); this.hideWarningClass = ""; this.warning_message = JSON.parse(err._body).message; }
        },
        () => console.log());
    }
    else {
      this.companyDetail.cin = '';
      this.companyDetail.doi = ''
    }
  ...
  onSubmitBusinessDetail() {
    this.loaderClasssubmit = "addLoader";
    let createBusinessDetail = this.business_detail.value;
    if (this.companyDetail.cin != '') createBusinessDetail.cin = this.companyDetail.cin;
    createBusinessDetail.registered_business_name = this.companyDetail.name;
    createBusinessDetail.registered_business_address = this.companyDetail.registered_business_address;

    if (this.companyDetail.doi != '') {
      let inDate = this.companyDetail.doi;
      var dateParts = inDate.split(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
      createBusinessDetail.date_of_incorporation = dateParts[3] + "-" + dateParts[2] + "-" + dateParts[1];
    } else {
      createBusinessDetail.date_of_incorporation = this.datePipe.transform(createBusinessDetail.date_of_incorporation, 'yyyy-MM-dd');
    }

    let business_pan = {
      'name': (this.companyDetail.name != '') ? this.companyDetail.name : createBusinessDetail.brand_name,
      'doi': createBusinessDetail.date_of_incorporation,
      'pan_number': createBusinessDetail.business_pan.toUpperCase()
    }
    createBusinessDetail.gstin_number = createBusinessDetail.gstin_number.toUpperCase();
    createBusinessDetail.business_pan = business_pan;
    createBusinessDetail.companies_id = this.companies_id;
    createBusinessDetail.accounts_id = this.accounts_id;
    createBusinessDetail.current_step = 1;
    createBusinessDetail.current_sub_step = 1;


    this._httpService.business_details(createBusinessDetail)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 200) {
          mixpanel.track('business-tab-save-proceeded', createBusinessDetail);
          Intercom('trackEvent', 'business-tab-save-proceeded', createBusinessDetail);
          this.getMobData();
        }
      },
      (err: any) => {
  ...
  createDirectorDetails() {
    let final_director_detail = [];
    let directorDetail = JSON.parse(localStorage.getItem('director_detail'));
    for (let d of directorDetail) {
      var dd = {
        "din": d.din,
        "designation": d.designation,
        "date_of_appointment": d.dateOfAppointment,
        "dsc_expiry_date": d.dscExpiryDate,
        "name": d.name,
        "whether_dsc_registered": d.whetherDscRegistered,
        "address_line": d.splitAddress.addressLine,
        "address_district": d.splitAddress.district[0],
        "address_state": d.splitAddress.state[0][0],
        "address_city": d.splitAddress.city[0],
        "pincode": d.splitAddress.pincode,
        "country": d.splitAddress.country[2],
        "father_name": d.fatherName,
        "dob": d.dob,
        "pan": d.pan
      }
      final_director_detail.push(dd);
    ...
    let data = {
      ...
      "director_details": final_director_detail
    }



    this._httpService.create_director_details(data)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
  ...
  onSubmitUpdateBusinessDetail() {
    ...
    // let businessDetailId = this.mob_data.mob_business_details.data.mob_business_details_id;

    // //let userId = this.mob_data.mob_business_details.data.mob_business_details_id
    // // this._httpService.updateBusinessDetail(data, businessDetailId)
    // //   .subscribe((result) => {
    // //     if (result.status == 200) {
    // //       this.getMobData();
    // //     }
    ...
    data.business_pan.files_id = [this.businesspan_data.id];
  ...
  verifyAadhaarCard(type) {
    if (type == 2) {
      //close the aadhaar verification detail modal
      //open verification popup again
      this.modalAccordionClose.nativeElement.click();
      this.verifydetails.nativeElement.click();
    }
    this.loaderVerifyingAadhaar = "uploadStart";
    let aadhaar_id = this.mob_data.mob_stakeholder_details.data.aadhaar_details.data.id;
    this._httpService.verifyAddhar(aadhaar_id)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 200) {
          this.loaderVerifyingAadhaar = '';
          this.aadhaar_verified_status = result.data.verified;
          if (this.aadhaar_verified_status) {
            this.verification_popup_heading = 'Verification Success';
            this.verification_popup_image = 'verify-details-green';
            this.pinkloader = "aniamteThree";
            setTimeout(() => {
              this.verifydetailsClose.nativeElement.click();
            }, 1000);
          }
        }
      },
  ...
  createAddressDetail() {
    let rba = this.mob_data.roc_registered_business_address.data;
    let data = {
      "mob_master_id": this.mob_data.mob_master_id,
      "current_step": 2,
      "current_sub_step": 4,
  ...
  onSubmitBusinessAddress() {
    this.loaderClasssubmit = "addLoader";
    let operation_address_is_diffrent = this.showBusinessAddressForm;
    let rba = this.mob_data.roc_registered_business_address.data;
    ...
    data.mob_master_id = this.mob_data.mob_master_id;
    if (rba.address_line != '' && rba.city != '' && rba.pincode != '') {
    ...
    if (operation_address_is_diffrent) {
      data.is_operational_address_different = 1;
      data.operation_address = data.operation_address;
      data.address_proof.files_id = this.operation_address_proofs_files_id;
    } else {
      data.is_operational_address_different = 0;
      data.operation_address = data.registered_address;
      data.address_proof.files_id = this.operation_address_proofs_files_id;
    }
    let addressDetailId = this.mob_data.mob_address_details.data.mob_address_details_id;
    this._httpService.updateAddressDetail(data, addressDetailId)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 200) {
          mixpanel.track('address-tab-save-proceeded', data);
          //data.business_address_proof_urls = this.address_proof;
  ...
  onSubmitAddSignatory() {
    this.loaderClasssubmit = "addLoader";
    let data = this.add_signatory.value;
    data.add_signatory_dob = this.datePipe.transform(data.add_signatory_dob, 'dd/MM/yyyy');
    ...
    let new_director = [
      {
        ...
        "date_of_appointment": "",
        "dsc_expiry_date": "",
        "name": data.add_signatory_first_name + " " + data.add_signatory_middle_name + " " + data.add_signatory_last_name,
        ...
        "country": "",
        "father_name": "",
        "dob": data.add_signatory_dob,
        "pan": data.add_signatory_pan_number
      }
    ];
    ...
    let new_data = {
      ...
      "current_sub_step": 2,
      'mob_master_id': this.mob_data.mob_master_id,
      'director_details': new_director
    };

    this._httpService.create_director_details(new_data)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 201) {
          this.closeAddSingatoryModel.nativeElement.click();
          //this.loaderClasssubmit = "";
          this.add_signatory.reset();
          this.getMobData();
        }
      },
      (err: any) => {
  ...
  onSubmitAadhaarUpdateDetail() {
    this.loaderClasssubmit = "addLoader";
    let data = this.aadhaarUpdateDetail.value;
    data.accounts_id = this.accounts_id;
    data.companies_id = this.companies_id;
    data.aadhaar_dob = this.datePipe.transform(data.aadhaar_dob, 'yyyy-MM-dd');
    let mob_stakeholder_details_id = this.mob_data.mob_stakeholder_details.data.mob_stakeholder_details_id;
    this._httpService.updateAadhaarDetails(data, mob_stakeholder_details_id)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 200) {
          this.aadhaar_verified_status = true;
          this.verifyAadhaarCard(2);
        }
      },
      (err: any) => {
        this.loaderClasssubmit = "";
  ...
  onSubmitCertify() {
    this.loaderAlmostDone = "addLoader";
  ...
  verifyingDetail() {
    this.verifydetails.nativeElement.click();
    this.verifyAadhaarCard(1);
  }

  retryVerifyDetail(type) {
    if (type == 3) {
      this.aadhaar_detail = this.mob_data.mob_stakeholder_details.data.aadhaar_details.data;
      this.aadhaarUpdateDetail.patchValue({ aadhaar_address: this.aadhaar_detail.aadhaar_address });
      this.showAadhaarUpdateDetailForm = true;
      this.modalAccordion.nativeElement.click();
      this.modalAccordionFadeClass = 'in';
    }
  }

  onAadhaarVerificationModalClose() {
    this.verifydetails.nativeElement.click();
  }

  //business pan image upload from system
  onChangeBusineesPan(fileInput: any) {
    ...
    if (fileInput.target.files && fileInput.target.files[0]) {
      ...
      } else { this.adobeIconForBusinessPan = false; }
      ...
      this._httpService.businessPan(imgSrc)
        .subscribe((result) => {
          ...
          if (result.status == 200) {
            ...
            this.businesspan_data = result.data;
            this.businesspan_data.meta = result.meta;
            this.getMobData();
          }
        },
        (err: any) => {
  ...
  saveBusinessPan() {
    if (this.mob_data.mob_director_details.data.length == 0) {
      if (localStorage.getItem('director_detail')) {
        this.createDirectorDetails();
      }
    }
  ...
  //End- business pan image upload from webcam

  onSelectAuthorisedSignatory(event) {
    this.authorised_signatory_selected = event;
  }

  onSubmitAuthorisedSignatory() {
    this.onbord_flag = 2;
    this.onbord_form_flag = 2;
    this.update_onbord_flag = 0;
    this.update_onbord_form_flag = 0;
  ...
  //signtory's pan image upload from system
  ...
  //signtory's pan image upload from webcam
  captureMultipartPan() {
    this.closeBtn.nativeElement.click();
    return this.webcam.captureAsFormData({ fileName: 'file.png' })
      .then(base => {
  ...
  //End - signtory's pan image upload from webcam

  //address_proof image upload from system
      ...
      for (let imgSrc of fileInput.target.files) {
        this._httpService.fileUploadOthers(imgSrc)
          .subscribe((result) => {
            if (result.status == 200) {
              result = result.data;
              this.address_proof.push(result);
              this.current_address_proofs_files_id.push(result.id);
            }
          },
          (err: any) => {
            this.hideWarningClass = "";
  ...
  onOperationalAdress(fileInput: any) {
    if (fileInput.target.files && fileInput.target.files[0]) {
      let imgSrc = fileInput.target.files[0];
      this._httpService.fileUploadOthers(imgSrc)
        .subscribe((result) => {
          ...
          if (result.status == 200) {
            result = result.data;
            this.operation_address_proofs.push(result);
            this.operation_address_proofs_files_id.push(result.id);
          }
        },
        (err: any) => {
          this.hideWarningClass = "";
  ...
  //delete other images
  delteOtherFiles(data, type) {

    if (data && type == 'currentAddress') {
    ...
    if (data && type == 'operationAddress') {
      this.operation_address_proofs.splice(this.operation_address_proofs.indexOf(data), 1);
      this.operation_address_proofs_files_id.splice(this.operation_address_proofs_files_id.indexOf(data.id), 1);
    }
  }
  ...
  toggleCurrentAddressForm(value) {
    if (value) {
      this.showCurrnetAddressForm = true;
      this.businessAddressProof = true;
    } else {
      this.showCurrnetAddressForm = false;
      this.businessAddressProof = false;
    }
  }

  toggleBusinessAddressForm(value) {
    if (value) {
      this.is_operation_addr_diff = true;
      this.showBusinessAddressForm = true;
    } else {
      this.is_operation_addr_diff = false;
  ...
  getMobData() {
    ...
    this._httpService.getMobDetails()
      .subscribe((result) => {
        ...
        if (typeof this.mob_data != "undefined" && typeof this.mob_data.mob_business_types_id != "undefined" && this.mob_data.mob_business_types_id != 4) {
        ...
        if (typeof this.mob_data != 'undefined' && this.mob_data.hasOwnProperty('mob_business_details') && this.mob_data.mob_business_details) {
          ...
          if (this.mob_data.current_step == 1 && this.mob_data.current_sub_step == 1) {
            ...
            this.director_details = this.mob_data.mob_director_details.data;
            this.onbord_flag = 2;
            ...
            this.onbord_flag = 2;
            this.onbord_form_flag = 4;
          } else if (this.mob_data.current_step == 2 && this.mob_data.current_sub_step == 4) {
            this.registered_business_detail = this.mob_data.roc_registered_business_address.data;
            this.onbord_flag = 3;
            this.onbord_form_flag = 1;
          } else if (this.mob_data.current_step == 3 && this.mob_data.current_sub_step == 1) {
            this.onbord_flag = 4;
            this.onbord_form_flag = 1;
            // if (this.mob_data.mob_stakeholder_details.data.aadhaar_details.data.has_verified == null || this.mob_data.mob_stakeholder_details.data.aadhaar_details.data.has_verified == 0) {
            //   this.verifyingDetail();
            // }
  ...
  ////////////////////////////////// UPDATE CODE START ///////////////////////////
  ////////////////////////////All code dedicated only for PVLTD OB///////////////
  ...
  update_click(current_step, current_sub_step, update_weight) {
    if (typeof this.mob_data != 'undefined') {
      ...
      if (update_weight <= mob_weight) {
        ...
        //This condition mob_address_details dedicated only for PVLTD 
        if (this.mob_data.hasOwnProperty('roc_registered_business_address')) {
          if (this.mob_data.roc_registered_business_address.hasOwnProperty('data')) {
            this.registered_business_detail = this.mob_data.roc_registered_business_address.data;
            let registerd_address = {
              "address_line": this.mob_data.roc_registered_business_address.data.address_line,
              "district": this.mob_data.roc_registered_business_address.data.district,
              "city": this.mob_data.roc_registered_business_address.data.city,
              "state": this.mob_data.roc_registered_business_address.data.state,
              "pincode": this.mob_data.roc_registered_business_address.data.pincode,
              "country": this.mob_data.roc_registered_business_address.data.country
            };
            this.companyDetail.registered_business_address = registerd_address;
        ...
        if (this.mob_data.hasOwnProperty('mob_address_details')) {
          if (this.mob_data.mob_address_details.data.is_operational_address_different) {
            this.is_operation_addr_diff = true;
            this.showBusinessAddressForm = true;
            this.operation_address_data = {
              "address_line": this.mob_data.mob_address_details.data.operation_address.data.address_line,
              "city": this.mob_data.mob_address_details.data.operation_address.data.city,
              "state": this.mob_data.mob_address_details.data.operation_address.data.state,
              "pincode": this.mob_data.mob_address_details.data.operation_address.data.pincode,
              "address_proofs_id": this.mob_data.mob_address_details.data.document_proof_files.data[0].address_proof_types_id
            };

            let data = this.mob_data.mob_address_details.data.document_proof_files.data;
            this.operation_address_proofs = [];
            this.operation_address_proofs_files_id = [];
            data.forEach(e => {
              if (e.address_proof_types_id != 18 && e.address_proof_types_id != 19 && e.address_proof_types_id != 23 && e.address_proof_types_id != 24) {
                this.operation_address_proofs.push(e.files.data[0]);
                this.operation_address_proofs_files_id.push(e.files.data[0].id);
              }
            });
        ...
        if (this.mob_data.hasOwnProperty('mob_business_details')) {
          this.ucompanyName = {
            "companyName": this.mob_data.mob_business_details.data.registered_business_name
          };
          this.companyDetail.name = this.mob_data.mob_business_details.data.registered_business_name;
          if (this.mob_data.mob_business_details.data.hasOwnProperty('business_pan_details')) {
            if (this.mob_data.mob_business_details.data.business_pan_details.data.hasOwnProperty('document_proof_files')) {
              this.businesspan_data = {
              ...
              // this.businesspan_data = {
              //   "id": (typeof this.businesspan_data != 'undefined') ? this.businesspan_data.id : '',
              //   "url": (typeof this.businesspan_data != 'undefined') ? this.businesspan_data.url : '',
              //   "meta": {
              //     "name": this.mob_data.mob_business_details.data.registered_business_name,
              //     "dateOfIssue": this.mob_data.mob_business_details.data.business_pan_details.data.doi,
              //     "number": this.mob_data.mob_business_details.data.business_pan_details.data.pan_number
        ...
        if (this.mob_data.hasOwnProperty('mob_director_details')) {
          if (this.mob_data.mob_director_details.hasOwnProperty('data')) {
            if (this.mob_data.mob_director_details.data.length > 0) {
              this.director_details = this.mob_data.mob_director_details.data;
            }
          }
  ...
  updateonSubmitBusinessDetail() {
    this.loaderClasssubmit = "addLoader";
    let createBusinessDetail = this.business_detail_bupdate.value;
    if (this.companyDetail.cin != '') createBusinessDetail.cin = this.companyDetail.cin;
    createBusinessDetail.registered_business_name = this.companyDetail.name;
    createBusinessDetail.registered_business_address = this.companyDetail.registered_business_address;
    if (this.companyDetail.doi != '') {
    ...
    let id = this.mob_data.mob_business_details.data.mob_business_details_id;
    this._httpService.updateBusinessDetail(createBusinessDetail, id)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
  ...
  updateonSubmitUpdateBusinessDetail() {
    this.loaderClasssubmit = "addLoader";
    let data = this.business_detail_update.value;
    data.companies_id = this.companies_id;
    data.accounts_id = this.accounts_id;
  ...
  updateonSubmitBusinessAddress() {
    ...
    if (operation_address_is_diffrent) {
      ...
      data.is_operational_address_different = 0;
      data.operation_address = data.registered_address;
      data.address_proof.address_proofs_id = "";
      data.address_proof.files_id = [];
