...
@Component({
  selector: 'onboard-proprietership',
  templateUrl: './proprietership.component.html',
  animations: [
    trigger(
      'myAnimation',
      [
...
export class ProprietershipComponent implements OnInit {
  public companies_id;
  public accounts_id;
  public users_id;
  public pan_aadhar;
  ...
  public business_address_proof_file_url = "";
  ...
  business_detail_update = new FormGroup({
    mob_business_types_id: new FormControl({ value: this.mob_data.mob_business_types_id, disabled: true }),
    registered_business_name: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-z0-9 ]{1,100}$')]),
    brand_name: new FormControl('', [Validators.maxLength(100)]),
    date_of_incorporation: new FormControl(''),
    gstin_number: new FormControl('', [Validators.pattern('^[0-9]{2}[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}[1-9A-Za-z]{1}[Z-z]{1}[0-9A-Za-z]{1}$')]),
  ...
  onChangeAddressProofOthers(fileInput: any) {
    this.statusBusineesAddress = "uploadStart";
    if (fileInput.target.files && fileInput.target.files[0]) {
      for (let imgSrc of fileInput.target.files) {
        //let imgSrc = fileInput.target.files[0];
        this._httpService.fileUploadOthers(imgSrc)
          .subscribe((result) => {
            this.hideWarningClass = "clickHide";
            if (result.status == 200) {
              this.statusBusineesAddress = "uploadComplete";
      ...
      this._httpService.deleteFiles(id)
        .subscribe((result) => {
          ...
          if (result.status == 200) {
            if (step == 1) {
              this.update_onbord_flag = 2;
              this.update_onbord_form_flag = 1;
              this.pan_data = ''; //temp
              this.statusUploadPan = "";
  ...
  savePan() {
    let gstin = this.mob_data.mob_business_details.data.gstin_number;
    var currentUser = JSON.parse(localStorage.getItem('currentUser'));
    currentUser.company_details.company_gstin = gstin;
  ...
  getMobData() {
    ...
    this._httpService.getMobDetails()
      .subscribe((result) => {
        ...
        if (typeof this.mob_data != "undefined" && typeof this.mob_data.mob_business_types_id != "undefined" && this.mob_data.mob_business_types_id != 2) {
        ...
        if (typeof this.mob_data != "undefined" && typeof this.mob_data.mob_stakeholder_details != "undefined" && this.mob_data.mob_stakeholder_details.data.aadhaar_details) {
          this.statusUploadFront = "uploadComplete";
        }
        if (typeof this.mob_data != "undefined" && typeof this.mob_data.mob_address_details != "undefined") {
          this.statusBusineesAddress = "uploadComplete";
        }
        ...
        if (result.hasOwnProperty('meta')) {
          result.meta.forEach(e => {
            ...
            if (e.file_type == 1) this.business_address_proof_file_url = e.url;
  ...
  update_click(current_step, current_sub_step, update_weight) {
    if (typeof this.mob_data != 'undefined') {
      ...
      this.onbord_flag = 0;
      this.onbord_form_flag = 0;
      //if (current_step <= this.mob_data.current_step && current_sub_step <= this.mob_data.current_sub_step) {
  ...
  updateonSubmitBusinessDetail() {
    ...
    data.date_of_incorporation = this.datePipe.transform(data.date_of_incorporation, 'yyyy-MM-dd');
  ...
  updateonSubmitStakeholerAddress() {
    ...
    if (this.stakeholder_address.valid) {
      ...
      // data.current_step = 2;
      // data.current_sub_step = 3;
      // data.aadhaar_front_files_id = this.aadhar_data_back.id;
      // data.aadhaar_back_files_id = this.aadhar_data_front.id;
