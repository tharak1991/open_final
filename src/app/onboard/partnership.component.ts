...
export class PartnershipComponent implements OnInit {
  ...
  public adobeIcon: boolean = false;
  ...
  public loaderClassPartnershipDeed = "";
  ...
  public statusUploadPartnershipDeed = "";
  ...
  public partnership_deed_file_url = "";
    ...
    this._httpService.createBusinessAddress(data)
      .subscribe((result) => {
        ...
        if (result.status == 200) {
          ...
          data.business_address_proof_urls = this.business_address_proof;
  ...
  onSubmitBusinessDetailUpdate() {
    ...
    data.partnership_deed_files_id = [this.partner_deed_data.id];
  ...
  //partner deed image upload from system
  onChangePartnerDeed(fileInput: any) {
    this.loaderClassPartnershipDeed = "addLoader";
    this.statusUploadPartnershipDeed = "uploadStart";
    if (fileInput.target.files && fileInput.target.files[0]) {
      if (fileInput.target.files[0].type == "application/pdf") {
        this.adobeIcon = false;
      } else {
        this.adobeIcon = false;
      }
      ...
      this._httpService.fileUploadOthers(imgSrc)
        .subscribe((result) => {
          this.hideWarningClass = "clickHide";
          if (result.status == 200) {
            this.loaderClassPartnershipDeed = "";
            this.statusUploadPartnershipDeed = "uploadComplete";
            this.partner_deed_data = result.data;
            this.partner_deed_data.meta = result.meta;
            //this.getMobData();
          }
        },
        (err: any) => {
  ...
  onChangeAddressProofOthers(fileInput: any) {
    this.statusUploadBusinessProof = "";
    if (fileInput.target.files && fileInput.target.files[0]) {
      for (let imgSrc of fileInput.target.files) {
        //let imgSrc = fileInput.target.files[0];
        this._httpService.fileUploadOthers(imgSrc)
        ...
        .subscribe((result) => {
          ...
          if (result.status == 200) {
            if (step == 1) {
              ...
              this.partner_deed_data = ''; //temp
              this.adobeIcon = false;
              this.loaderClassPartnershipDeed = ""
              this.statusUploadPartnershipDeed = "";
            }
            else if (step == 7) {
  ...
  onSubmitStakeholerDetail() {
    ...
    this._httpService.stakeholderPan(data)
      .subscribe((result) => {
        this.hideWarningClass = "clickHide";
        if (result.status == 200) {
          mixpanel.track('stakeholer-tab-save-proceeded', data);
          data.stakeholder_pan_url = this.pan_data.url;
          Intercom('trackEvent', 'stakeholer-tab-save-proceeded', data);
          this.getMobData();
        }
      },
      (err: any) => {
  ...
  getMobData() {
    ...
    this._httpService.getMobDetails()
      .subscribe((result) => {
        ...
        if (typeof this.mob_data != "undefined" && typeof this.mob_data.mob_business_types_id != "undefined" && this.mob_data.mob_business_types_id != 3) {
            ...
            result.meta.forEach(e => {
              if (e.hasOwnProperty('intercom')) {
                ...
                window.intercomSettings = {
                  ...
                  pan_no: result.data.mob_stakeholder_details.data.individual_pan_details.data.individual_pan_number,
        ...
        if (result.hasOwnProperty('meta')) {
          result.meta.forEach(e => {
            if (e.file_type == 1) { this.businees_address_file_url = e.url; this.statusUploadBusinessProof = "uploadComplete" }
            ...
            if (e.file_type == 21) { this.partnership_deed_file_url = e.url; this.statusUploadPartnershipDeed = "uploadComplete" }
            if (e.file_type == 23) {
              this.aadhar_e_data_content = true;
      ...
      if (update_weight <= mob_weight) {
        ...
        if (this.mob_data.hasOwnProperty('mob_business_details')) {
          if (this.mob_data.mob_business_details.data.hasOwnProperty('business_pan_details')) {
            ...
            if (this.mob_data.mob_business_details.data.hasOwnProperty('document_proof_files')) {
              this.partner_deed_data = {
                "id": this.mob_data.mob_business_details.data.document_proof_files.data[0].files.data[0].id,
                "url": this.mob_data.mob_business_details.data.document_proof_files.data[0].files.data[0].url
              }
              if (this.mob_data.mob_business_details.data.document_proof_files.data[0].files.data[0].mime_type == "application/pdf") {
                this.adobeIcon = true;
              }
            }
          }
