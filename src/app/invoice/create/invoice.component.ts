...
import { Component, OnInit, ViewChild, ElementRef, ViewContainerRef, EventEmitter ,ChangeDetectorRef} from '@angular/core';
...
export class InvoiceComponent implements OnInit {
	...
	constructor(private datePipe :DatePipe,private _httpService:InvoiceService,private router:Router,private cdRef: ChangeDetectorRef,) 
	{
	...
	//	today_date=this.datePipe.transform(today_date, 'yyyy-MM-dd');//toLocaleString();
		this.store.invoice.date =yyyy+'-'+mm+'-'+dd;


		this.todaysDate1.setHours(0,0,0,0); //minDate
	...
	getAllItem(){
		...
		subscribe((result)=>{
			...
			data.forEach(element => {
				...
				if (this.items) {
					this.items.push(new Item(element.id,element.item_name,element.item_code,element.hsn_sac_code,type,'1',element.description, "","0","0",element.price,element.gst_perc,addtex,0,element.is_service,false));
				} else {
					this.items = [
					new Item(element.id,element.item_name,element.item_code,element.hsn_sac_code,type,'1',element.description, "" ,"0","0",element.price,element.gst_perc,addtex,0,element.is_service,false)
					];
				}
			});
		});
	...
	generateSequnceID(){
		...
		this._httpService.getSequenceId(data).subscribe((result)=>{
		   this.store.invoice.id=result.invoice_sequence_id;
		  // this.my_state=result.state;
		   this.hideWarningClass = "clickHide";
		},(err:any)=>{
			this.errorHandle(err);
		}, () => console.log());
			...
			this._httpService.fileUploadOthers(imgSrc)
			  .subscribe((result) => {
				if (result.status == 200) {
					...
					mixpanel.track('one-time-attach-files-clicked');
					Intercom('trackEvent','one-time-attach-files-clicked');
	...
	addNotesandTags(){
		if(typeof this.addNotesForm.value.tags !='undefined' && this.addNotesForm.value.tags.length >0){
			this.addNotesForm.value.tags.forEach(element => {
				this.tags.push(element);
			});
		}

		if(typeof this.addNotesForm.value.notes !='undefined'){
	...
	onSubmitCreateInvoice(data){
		...
		if(this.invoice_form.valid && this.store.items.length!=0){
			...
			this._httpService.createInvoice(udata)
			.subscribe(result=>{
				if(result.status==200){

					if(this.store.type=='1'){
						mixpanel.track('one-time-invoice-created',udata);
						Intercom('trackEvent','one-time-invoice-created',udata);
					}else{
						mixpanel.track('recurring-invoice-created',udata);
						Intercom('trackEvent','recurring-invoice-created',udata);
					}
					if(this.save_type=='1'){
